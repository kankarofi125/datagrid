...
export function BottomTabBar() {
  ...
  return (
    ...
    >
      <ul className="mx-auto flex max-w-lg items-end justify-between px-2 pt-1">
        {tabs.map((tab) => {
          const active =
            path === tab.href ||
          ...
          if (tab.fab) {
            return (
              <li key={tab.href} className="-mt-5">
                <Link
                  href={tab.href}
                  className="flex h-14 w-14 flex-col items-center justify-center rounded-full bg-green text-white shadow-lg shadow-green/30 pressable"
                  aria-label="Wallet"
                >
                  <tab.icon active />
                  <span className="font-mono-num mt-0.5 text-[9px] tracking-wide">
                    ₦
                  </span>
                </Link>
              </li>
...
function WalletIcon({ active: _active }: { active?: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden>
      <rect x="3" y="6" width="18" height="13" rx="2" stroke="white" strokeWidth="1.6" />
      <path d="M15 12.5h4" stroke="white" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}
